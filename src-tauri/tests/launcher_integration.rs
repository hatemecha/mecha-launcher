...
fn prepare_launch_uses_redux_game_directory_when_manifest_declares_it() {
    ...
    let game_dir_json = game_dir.to_string_lossy().replace('\\', "/");
    write_version(
        &minecraft_dir,
        "1.16.5-redux",
        &format!(
    ...
    let game_dir_for_args = game_dir.to_string_lossy().replace('\\', "/");
    assert!(plan
        .game_args
        .windows(2)
        .any(|pair| pair == ["--gameDir", game_dir_for_args.as_ref()]));
}
